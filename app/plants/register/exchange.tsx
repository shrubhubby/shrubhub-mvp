...
export default function ExchangeRegistration() {
  ...
  const [exchangedWith, setExchangedWith] = useState('')
  const [whatYouGave, setWhatYouGave] = useState('')
  ...
  const handlePhotoTaken = (capturedPhoto: CapturedPhoto) => {
    ...
    if (capturedPhoto.exif?.dateTaken) {
      setExchangeDate(capturedPhoto.exif.dateTaken.toISOString().split('T')[0])
    }
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('plants').insert({
        ...
        acquired_date: exchangeDate,
        acquisition_source: 'shrubhub_exchange',
        acquisition_location: exchangedWith.trim() || null,
        acquisition_notes: [
          whatYouGave && `Traded for: ${whatYouGave}`,
          notes,
        ].filter(Boolean).join('\n') || null,
        status: 'vegetative',
      ...
      Alert.alert('Success!', `Added ${plantName} from exchange.`, [
  ...
  return (
    ...
    >
      <View className="flex-1 bg-soft">
        <Header title="ShrubHub Exchange" showBack />

        <ScrollView className="flex-1" contentContainerStyle={{ padding: 16 }}>
          <View className="gap-4">
            <View>
              <Text className="text-xl font-bold text-coal">🔄 ShrubHub Exchange</Text>
              <Text className="text-sm text-coal/60 mt-1">
                Register a plant from a community exchange
              </Text>
            </View>
            ...
            <Card className="bg-forest/5 border-forest/20">
              <CardContent>
                <Text className="text-sm text-forest">
                  🌱 Plant exchanges help our community grow! Trading plants is
                  a great way to diversify your garden and share the joy of
                  gardening.
                </Text>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="gap-4">
                <Input
                  label="Plant Name *"
                  placeholder="What plant did you receive?"
                  value={plantName}
                ...
                <Input
                  ...
                  placeholder="ShrubHub username or name"
                  value={exchangedWith}
                  onChangeText={setExchangedWith}
                />

                <Input
                  label="What did you give in return?"
                  placeholder="e.g., Spider Plant cutting"
                  value={whatYouGave}
                  onChangeText={setWhatYouGave}
                />
                ...
                <Input
                  ...
                  placeholder="Care tips received, variety info, etc."
                  value={notes}
