  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('plants').insert({
        ...
        acquisition_location: giftedBy.trim() || null,
        acquisition_notes: [
  ...
  return (
    ...
    >
      <View className="flex-1 bg-soft">
        <Header title="Register Gift" showBack />

        <ScrollView className="flex-1" contentContainerStyle={{ padding: 16 }}>
          <View className="gap-4">
            <View>
              <Text className="text-xl font-bold text-coal">🎁 Add Gift Plant</Text>
              <Text className="text-sm text-coal/60 mt-1">
                Record a plant you received as a gift
              </Text>
            </View>
            ...
            <Card>
              <CardContent className="gap-4">
                <Input
                  label="Plant Name *"
                  placeholder="e.g., Orchid, Peace Lily"
                  value={plantName}
